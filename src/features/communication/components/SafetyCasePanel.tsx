  ...
  return (
    <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-4">
      ...
      <div className="mb-4 grid gap-3 sm:grid-cols-2">
        <div>
          <p className="text-[11px] uppercase text-alygo-text-muted">
            SOS Trigger
          </p>
          <p className="text-sm text-white">
            {SAFETY_TYPE_LABELS[safety.reportType] ?? safety.reportType}
          </p>
        </div>
        <div>
          <p className="text-[11px] uppercase text-alygo-text-muted">
      ...
      <div className="mb-4">
        <p className="mb-2 text-[11px] uppercase text-alygo-text-muted">
          Safety Timeline
        </p>
        <Timeline
          className="mt-0!"
          items={[
            {
              children: (
                <span className="text-sm text-white">
                  Report received —{" "}
                  {new Date(safety.lastActivity).toLocaleTimeString()}
                </span>
              ),
            },
            {
              children: (
                <span className="text-sm text-white">
                  Assigned to {safety.assignedAgent}
                </span>
              ),
            },
            {
              children: (
                <span className="text-sm text-white">
                  Live monitoring active on trip {safety.tripId}
                </span>
              ),
            },
          ]}
