      ...
      children: item.children
        ...
        .map((child) => ({
          ...
          children: child.children?.filter(
            (nested) => !nested.permission || hasPermission(nested.permission),
          ),
        })),
    }))
...
function buildMenuItems(
  items: ReturnType<typeof filterNavByPermissions>,
): MenuProps["items"] {
...
export function Sidebar() {
  ...
  const navItems = useMemo(
    () => filterNavByPermissions(NAVIGATION, hasPermission),
    [hasPermission],
  );
  ...
  const selectedKeys = useMemo(() => {
    ...
    const pathMatch = flat.find(
      (item) => item.path?.startsWith(pathname) && item.path.includes("?tab="),
    );
      ...
      <div className="flex h-16 items-center justify-between border-b border-white/5 px-4">
        <Link
          ...
          className={cn(
            ...
            collapsed ? "mx-auto" : "w-auto opacity-100",
          )}
        >
          {collapsed ? (
